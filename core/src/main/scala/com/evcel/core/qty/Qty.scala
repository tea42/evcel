...
object Qty {
  ...
  def apply(value: Double, uom: UOM) = new DblQty(value, uom)
  def apply(value: String, uom: UOM) = new BDQty(BigDecimal(value), uom)
  def apply(value: BigDecimal, uom: UOM) = new BDQty(value, uom)
}
