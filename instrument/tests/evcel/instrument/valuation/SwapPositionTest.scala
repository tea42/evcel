...
class SwapPositionTest extends ValuationTest {
  ...
  test("swap on futures contract index") {
    ...
    positions shouldEqual List(HedgeInfo("Nymex WTI nearby 1", PeriodLabel(ltd), swap.volume))
    val positionsEqF = swap.positions(undiscVC.copy(params = vc.params.withShowEqFutures(b = true)))
    positionsEqF.toList shouldEqual List(HedgeInfo(wti, PeriodLabel(oct), swap.volume))

    val discountRate = vc.discountRate(USD, settDay).toQty
    val positionsDisc = swap.positions(vc)
    positionsDisc assert_=== Vector(HedgeInfo("Nymex WTI nearby 1", PeriodLabel(ltd), swap.volume * discountRate))
    val positionsEqFDisc = swap.positions(vc.copy(params = vc.params.withShowEqFutures(b = true)))
    positionsEqFDisc assert_=== Vector(HedgeInfo(wti, PeriodLabel(oct), swap.volume * discountRate))
...
//    position.positions(fs, swap) shouldEqual Nil
  ...
  test("swap on published index") {
    ...
    positions shouldEqual Vector(HedgeInfo(sing, PeriodLabel(oct), swap.volume))
    ...
    swap.positions(vc.copy(params = vc.params.withTenor(Some(Day)))).toSet shouldEqual observationDays.map{
      d => HedgeInfo(sing, PeriodLabel(d), (swap.volume / observationDays.size).round(9))
    }.toSet
  }
  ...
  test("swap on futures front period index") {
    ...
    val positions = swap.positions(vc)
    positions shouldEqual HedgeInfo(wti1st, PeriodLabel(oct), V) :: Nil
    ...
    val expectedPositionsEqF = weights.map { case (m, w) =>
      HedgeInfo("Nymex WTI", PeriodLabel(m), (V * w).round(9))
    }
    positionsEqF shouldEqual expectedPositionsEqF

    val expectedDaily = observationDays.map { d =>
      HedgeInfo(wti1st, PeriodLabel(d), (V / observationDays.size).round(9))
    }.toSet
    val actual = swap.positions(vc.copy(params = vc.params.withTenor(Some(Day)))).toSet 
  ...
  test("nbp swap - thm/day") {
    ...
    for(period <- periods) {
      ...
      positions shouldEqual HedgeInfo(nbp1st, PeriodLabel(period), swap.volume) :: Nil
      positionsEnergy shouldEqual HedgeInfo(nbp1st, PeriodLabel(period), totalEnergy) :: Nil
      ...
      positionsEqF shouldEqual List(
        powerNov.map(q => HedgeInfo(nbp, PeriodLabel(nov), q.round(9))),
        powerDec.map(q => HedgeInfo(nbp, PeriodLabel(dec), q.round(9)))
      ).flatten

      val positionsEqFe = swap.positions(vcEnergyWithFuturesEq)
      positionsEqFe shouldEqual List(
        energyNov.map(e => HedgeInfo(nbp, PeriodLabel(nov), e.round(9))),
        energyDec.map(e => HedgeInfo(nbp, PeriodLabel(dec), e.round(9)))
      ).flatten
  ...
  test("nbp swap - thm/day - daily tenor") {
    ...
    val positions = swap.positions(vc)
    val positionsEnergy = swap.positions(vcEnergy)
    positions.toSet shouldEqual observationDays.map(d => HedgeInfo(nbp1st, PeriodLabel(d), swap.volume)).toSet
    positionsEnergy.toSet shouldEqual observationDays.map(d => HedgeInfo(nbp1st, PeriodLabel(d), dailyEnergy)).toSet
  }
